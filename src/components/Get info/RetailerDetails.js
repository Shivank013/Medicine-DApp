...
import Loading from "../Loading";
 ...
 function RetailerDetails(){
  ...
  const{getRetailerDetails} = useContext(AppContext);
  ...
  useEffect(()=>{
    const fetchData = async () => {
      ...
      setLoading(true);
      const data = await getRetailerDetails(inputValue);
      console.log(data);
      setdata(data);
      setLoading(false);
    };
  ...
  const handleClick = () => {
    ...
    return (
      <div className="bg-yellow-100 py-2 px-4 rounded-md border-black font-mullish text-md">
        <label htmlFor="inputField">Enter the Address of the Retailer:</label>
        <input
        ...
        { 
        !data ? (<div>Please enter aurthentic retailer address</div>) :
        !data[0]? (<div></div>) :
        (<div>
         <p>Retailer Address : {String(data[0])}</p>
         <p>Retailer Name : {data[1]}</p>
         <p>Retailer Location : {data[2]}</p>
