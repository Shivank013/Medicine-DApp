...
function Shopkeeper(){
  ...
  const {getShopkeeperDetails,createShopkeeper,addShopkeeper} = useContext(AppContext);
  
  const [shopkeeperName,setName] = useState('');
  const [shopkeeperLocation,setLocation] = useState('');
  const [shopkeeperReceivingDate,setDate] = useState('');
  ...
  useEffect(()=>{
    const fetchData = async () => {
      ...
      const data = await getShopkeeperDetails(accounts[0]);
      console.log(data);
      if(data){
      setNewUser(false);
      setWait(false);
    ...
    return (
        <div>
          { wait ? 
            ...
            <h2 className=" font-bold text-2xl self-center underline p-2 text-blue-600" >Register your self as a Pharmacist</h2>
            <div>
            ...
            <div>
            <label htmlFor="location">Pharmacist Loaction: </label>
            <input
            className=" text-black w-64 border-2 px-1 rounded-md font-medium"
            type="text"
            id="location"
            value={shopkeeperLocation}
            ...
            <input
            className=" text-black font-medium w-40 border-2 px-1 rounded-md"
            type="date"
            id="date"
            value={shopkeeperReceivingDate}
            ...
            (<form className=" bg-yellow-100 border-black flex flex-col mt-16 border font-mullish text-red-500 font-semibold rounded-lg shadow-inner justify-center p-5 px-7 gap-6 w-[45%] items-start mx-auto" onSubmit={add}>
              <h2 className=" font-bold text-2xl self-center underline p-2 text-blue-600">Add all medicines for tracking (Pharmacist)</h2>
